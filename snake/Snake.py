...
class Snake(object):
    ...
    def createOval(self):
        ...
        for i in range(4):
            corps = self.can.create_oval(x-self.rayon-i*self.rayon*2,y-self.rayon,x+self.rayon-i*self.rayon*2,y+self.rayon,fill=self.color_snake,outline=self.color_snake,tag="tete" if i == 0 else "corps")
    ...
    def animSnake(self):
        ...
        for i in self.can.find_withtag('tete'):
            ...
            for i in self.can.find_withtag('corps'):
                self.coords_snake.append(self.can.coords(i))
        if col == 1:
            corps = self.can.create_oval(fin,fill=self.color_snake,outline=self.color_snake,tag="corps")
            self.coords_snake.append(self.can.coords(corps))
        for i in self.can.find_withtag('corps'):
    ...
    def createFruit(self):
        ...
        fruit = self.can.create_oval(x-self.rayon,y-self.rayon,x+self.rayon,y+self.rayon,fill="blue",tag="fruit")
